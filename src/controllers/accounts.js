...
export const addAccount = async (req, res, next) => {
  try {
    ...
    const existingAccountIndex = user.accounts.findIndex(
    ...
    if (existingAccountIndex !== -1) {
      ...
      return res
        .status(201)
        .json({ success: true, message: "Account updated successfully", user });
    } else {
      // Add new account
      ...
      return res
        .status(201)
        .json({ success: true, message: "Account added successfully", user });
    }
  } catch (error) {
    next(error);
  }
...
>>>>>>> 92bd44928b35eaee9c00e951db7eb6bfc3fc26b5
